...
- 2026-06-19 追加对比：同一 Sui SUI -> Solana USDC route 在升级前 Mayan v1 +
  Sui v1 JSON-RPC build/dry-run 可通过；Mayan v2 + Sui v2 JSON-RPC compat build/dry-run
  也可通过；失败点集中在 Mayan v2 route 交给 Sui v2 gRPC/Core resolver 构造时。
  因此短期实现允许 bridge SDK 接收显式 `buildClient: legacyJsonRpc` 作为 Mayan
  third-party adapter 构造/normalize bytes，后续签名、Core simulate、execute 仍走注入的
  Sui v2 provider；不做 public endpoint 自动 fallback。
