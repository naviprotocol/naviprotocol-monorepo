...
2. 当前 `@naviprotocol/astros-bridge-sdk@2` 内置的 Mayan v1 provider 构建出的 bridge transaction bytes 可以被 v2 `Transaction.from(bytes)` 解析、v2 JSON-RPC dry-run、v2 signer 签名并执行成功。测试交易 `HRLeV1vA38xrtyiXxQT31o9NEBPGUjN6fJzLcuxU7Gc2` 已被 Mayan Explorer 标记 `COMPLETED`，open-aggregator 标记 `completed`；后续又用当前 SDK 依赖版本完成 `0.6 USDC` Sui -> Arbitrum USDC，source tx `AZENsYHb2x6tpzvehfJcJFv1r5pJjDEG3U4pTh4pjoDf`，dest tx `0xa810ef53f30600f7ffb24279c0d369e7efe1c5f31078e3762d4315ac22bfcd16`，SDK status `completed`。Bridge 最终方案是 `@naviprotocol/astros-bridge-sdk@2` 内部 lazy Mayan provider，不把 Bridge 迁到 open-api。
...
8. Bridge 已确认纳入 SDK v2 beta 首批交付，按 `@naviprotocol/astros-bridge-sdk@2` internal lazy Mayan provider 方案执行；验收必须包含 root entry 不加载 Mayan/Sui v1、lazy chunk、前端 build chunk 和多 route smoke。
...
1. `@naviprotocol/astros-bridge-sdk@2` public surface 声明 Sui v2 ready，但 Mayan v1 只作为 SDK 内部 lazy provider 存在；root entry 不同步加载 Mayan / Sui v1 依赖，不把 v1 类型暴露给前端。
...
Mayan 仍依赖 Sui v1，但已实测“v1 build 标准 bytes -> v2 parse / sign / execute/status”可行，因此不再作为 Bridge 产品功能或 bridge SDK v2 public package 的硬阻塞；风险收敛到 lazy provider 打包隔离、lazy load、以及更多 route / chain 的回归覆盖。
...
| `@mayanfinance/swap-sdk` | 当前 SDK beta 实现依赖 `13.3.0`；npm 最新 `14.2.0` 仍 dependencies `@mysten/sui ^1.34.0`；Mayan Sui 源链 tx build 依赖 SDK 本地拼 Move calls | `astros-bridge-sdk`、产品 Bridge action | P1 | `astros-bridge-sdk@2` public API 使用 Sui v2 类型；Mayan v1 放 SDK 内部 lazy provider，provider build 标准 bytes 后由 SDK 转 v2 `Transaction`、调用前端/外部传入的 v2 signer，并用 v2 client execute；`astros-bridge-sdk@1.x` legacy line 继续给旧前端维护 | 否；不等 Mayan v2，但要做 lazy bundle / 依赖树 / 多 route smoke |
...
| `@naviprotocol/astros-bridge-sdk@2` Mayan v1 provider | **SDK 内部 lazy provider chunk** | 前端只需要安装 `@naviprotocol/astros-bridge-sdk@2`，不应该额外感知 Mayan v1 包。当前 beta 的 `swap()` 执行路径会动态加载 `src/providers/mayan.ts` 对应 chunk；root entry 保持轻量，普通非 Bridge 页面不加载 Mayan / Sui v1。后续如果拆分 provider，可进一步做到按源链更细粒度 lazy load。 |
...
naviprotocol-monorepo/packages/astros-bridge-sdk/
  ...
  src/providers/mayan.ts    # lazy Mayan provider, build bytes -> v2 sign -> execute
  src/types.ts              # v2 wallet connection types and plain DTOs
```
...
// root import remains light; bridge execution loads the Mayan provider chunk
...
| --- | --- | --- |
| 依赖树 | root `@mysten/sui@2`，adapter package 内部 `@mysten/sui@1` | SDK public peer 只有 `@mysten/sui@2`；Mayan / Sui v1 只允许作为 lazy provider 的 build/runtime 实现，不出现在 public API |
...
| 后续替换 | 上游协议 v2 ready 后移除对应 adapter package | Mayan v2 ready 后替换 lazy provider，不改 public API |
...
| Bridge / Mayan swap | 构建跨链交易和链上 source tx | 不属于 Copilot，也不走 open-api。`astros-bridge-sdk@2` public API 走 v2，内部 lazy Mayan provider build 标准 bytes，SDK 转 v2 `Transaction` 后调用前端/外部传入的 signer，并由 SDK 使用 v2 client execute | 条件性，只能走 Bridge SDK lazy provider 或 Mayan v2 |
...
| Mayan Bridge | v1 Mayan build bytes -> v2 parse / dry-run / sign / execute / status 已完成 1 SUI -> Arbitrum USDC smoke；当前 SDK beta 又完成 0.6 USDC -> Arbitrum USDC execute/status | `astros-bridge-sdk@2` 内部 lazy Mayan provider；public API 不暴露 v1 类型；前端仍通过 bridge SDK 发起 Bridge，只提供 v2 wallet signer 和 v2 JSON-RPC provider | 否，不属于 Copilot |
...
### 3.7 Bridge SDK v2 + Mayan v1 lazy provider 方案
...
sequenceDiagram
  ...
  SDK->>Wallet: 触发签名或调用前端提供的 signer
...
1. `@naviprotocol/astros-bridge-sdk@2` 对外只暴露 v2 兼容 API、plain DTO、v2 JSON-RPC provider + wallet signer contract、metadata 和 status，不暴露 Mayan / Sui v1 类型。
2. Mayan v1 provider 是 SDK 内部实现，不单独要求前端安装。
3. root entry 不同步 import Mayan；当前 beta 的 `swap()` 执行路径动态 import `src/providers/mayan.ts` 对应 chunk，再在 provider 内按源链分支执行。
4. 当前 bridge backend / aggregator backend 不升级也可以继续工作，因为它们仍只提供 quote / route / status 数据；Sui source tx build 仍在 SDK 内完成。
5. 后续 Mayan 发布 v2 后，只替换 lazy provider，保持 SDK public API 不变。

实测结论：Mayan v1 SDK 虽然依赖 Sui v1，但它构建出来的是标准 Sui transaction bytes；v2 SDK 可以 `Transaction.from(bytes)` 解析，v2 JSON-RPC dry-run、v2 signer 签名和 `executeTransactionBlock` 均可以接收。已用测试钱包完成 1 SUI -> Arbitrum USDC smoke；当前 `astros-bridge-sdk@2` beta 依赖的 `@mayanfinance/swap-sdk@13.3.0` 又完成 0.6 USDC -> Arbitrum USDC execute/status：
...
| 当前 SDK beta source digest | `AZENsYHb2x6tpzvehfJcJFv1r5pJjDEG3U4pTh4pjoDf` |
| 当前 SDK beta dest tx | `0xa810ef53f30600f7ffb24279c0d369e7efe1c5f31078e3762d4315ac22bfcd16` |
| 当前 SDK beta status | `completed` |

这个 smoke 证明“内部 Mayan v1 builder -> 标准 bytes -> v2 Transaction -> v2 signer -> v2 execute/status”对 Mayan Sui 源链交易可行。它不覆盖所有 route / chain / quote type；正式改造仍需要把同样验证固化到 bridge SDK 集成测试。
...
| `@naviprotocol/astros-bridge-sdk@2 beta` | Sui v2 public API ready；内部 lazy Mayan v1 provider |
| `@naviprotocol/astros-bridge-sdk@1.x` | legacy bridge line，继续给旧前端维护 |

`astros-bridge-sdk@1.x` legacy line 需要做小改造。下面是 legacy 线可评估的版本建议，不代表当前 `astros-bridge-sdk@2 beta` 已切到 Mayan `14.2.0`；当前 beta 实现仍以 package 里的 `@mayanfinance/swap-sdk@13.3.0` 为准：
...
| `@mayanfinance/swap-sdk` | lazy provider 依赖，优先打进 lazy chunk；如果 build 工具无法完全 bundle，则作为普通 dependency 存在也必须只被 dynamic import 路径触达。 |
| Mayan 传递的 `@mysten/sui@1` | 只允许存在于 provider 依赖树 / lazy chunk，不允许出现在 public API 类型、root export、非 Bridge 页面 chunk。 |

打包配置要保证 Mayan v1 provider 只进入 lazy chunk，而不是 root entry。正式实现时要用 SDK build 产物和前端 app build 产物验证两件事：root import 不拉 Mayan，执行 Bridge swap 才加载 provider chunk。
...
execute Sui bridge
  -> dynamic import providers/mayan
  -> 加载 Mayan + Sui v1 依赖
```
...
当前已验证 1-6：1 SUI -> Arbitrum USDC source tx 已 v2 签名执行成功，Mayan 和 open-aggregator 状态均已完成；当前 SDK beta 依赖版本又完成 0.6 USDC -> Arbitrum USDC，SDK status `completed`。Bridge 产品功能因此不需要等待 Mayan 官方 v2 SDK；剩余工作是把 lazy provider、依赖树检查、前端 build chunk 检查和多 route smoke 固化到 SDK v2 回归。
...
2. Mayan：通过 `astros-bridge-sdk@2` 内部 lazy provider 承载，不纳入 open-api；funded wallet smoke 已通过，剩余工作是把 SDK lazy provider、依赖树检查、前端 chunk 检查和回归用例实现出来。
...
| `@mayanfinance/swap-sdk@14.2.0` | 最新版本仍依赖 `@mysten/sui ^1.34.0`；当前 SDK beta 实现仍使用 `13.3.0` | 未发现 Sui v2 migration issue / PR；open issue 主要是 Sui PTB 业务 bug | 无公开 ETA | 不等待上游 v2；`astros-bridge-sdk@2` 用 lazy provider 隔离 Mayan v1，当前实现已实测 build bytes 可被 v2 parse / sign / execute/status |
...
| 核心改造 | 发布 `astros-bridge-sdk@2`：public API 使用 Sui v2 类型和 plain DTO；Mayan v1 放在 `src/providers/mayan.ts` 对应 lazy chunk；provider 输出标准 transaction bytes 后立即转 v2 `Transaction`，调用前端/外部传入的 v2 signer，并由 SDK 使用 v2 JSON-RPC provider 执行源链交易；`astros-bridge-sdk@1.x` 继续作为旧前端 legacy line 维护。 |
| 阻塞依赖 | Mayan 不阻塞 SDK/open-api v2 主链路，也不阻塞产品 Bridge；风险集中在 lazy provider 打包隔离、root entry 不加载 Mayan、以及多 route smoke 覆盖。 |
...
| Bridge / Mayan tx build | 不属于 Copilot，也不迁 open-api；由 `@naviprotocol/astros-bridge-sdk@2` 内部 lazy Mayan provider 承载，前端/外部只提供 v2 wallet signer，SDK 用 v2 provider execute/status |
...
6. Bridge/Mayan lazy provider 需要把已通过的手工 smoke 固化为 bridge SDK 集成测试，并覆盖 SDK build、前端 build、root entry 不加载 Mayan、lazy chunk 和多 route smoke。
...
- [ ] Bridge 纳入 SDK v2 beta 首批交付；必须通过 `astros-bridge-sdk@2` lazy provider 回归测试覆盖已验证的 build bytes / v2 parse / v2 sign / execute / status 查询链路；root entry 不加载 Mayan / Sui v1，前端 app build chunk 检查通过。
...
1. **SDK / open-api 主链路落地**：先做 SDK Node 22.x / open-api Node 24.x、Sui v2、Core/GraphQL/JSON-RPC adapter、Pyth 自维护 v2 builder、Bridge SDK internal Mayan provider 回归，把已验证的路径固化进代码。
