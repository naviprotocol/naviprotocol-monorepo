...
NAVI Lending SDK is a lending SDK designed specifically for the Sui blockchain, providing complete lending functionality including account management, pool operations, market management, EMode operations, flash loans, liquidation, and reward systems.
...
Markets are organizational units that group related pools and EMode configurations together. Each market can have its own pools and EMode settings, allowing for better organization and management of different asset categories.

### EMode (Efficiency Mode)

EMode is a special lending mode that allows users to borrow at higher LTV (Loan-to-Value) ratios within specific asset categories, improving capital efficiency. Users can enter and exit EMode to optimize their lending strategies.
...
- `client`: Sui SDK v2 gRPC/Core client instance, such as `SuiGrpcClient` from `@mysten/sui/grpc`, used for customizing network requests in multi-network or custom RPC environments. (If you need a temporary legacy JSON-RPC compatibility path, use `SuiJsonRpcClient` from `@mysten/sui/jsonRpc` via `legacyJsonRpc` — it is deprecated and scheduled for removal on 2026-07-31.)
...
- `market`: Market identifier for operations that support multiple markets
