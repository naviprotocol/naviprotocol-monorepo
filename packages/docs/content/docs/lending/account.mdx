...
Get user's lending account information. You can filter by specific market(s). See [documentation](/typedoc/variables/_naviprotocol_lending.getLendingState.html) for details.

<Callout>
  We standardized the precision of the `borrowBalance` and `supplyBalance`, with all decimal places
  set to `9`.{' '}
</Callout>
...
<Callout>
  For a more structured way to manage positions with automatic value calculations, EMode support,
  and overview statistics, see the <a href="/lending/position">Position Management</a>{' '}
  documentation.
</Callout>
...
Verify that the user's health factor meets a minimum threshold in a programmable transaction block. This function calls the limiter contract's `verify_navi_position_healthy` method to ensure the account remains healthy after operations.

**Usage Example**

```typescript
import { verifyHealthFactorPTB } from '@naviprotocol/lending'
import { Transaction } from '@mysten/sui/transactions'

const tx = new Transaction()

// Verify health factor >= 1.1 after operations
await verifyHealthFactorPTB(tx, '0x...', 1.1)
...
- `tx`: Transaction object
- `address`: User address, Account Cap object ID, or TransactionResult
- `hf`: Minimum health factor threshold (e.g., `1.1`)
- `options` (optional):
  - `env`: Environment configuration ('dev' or 'prod')
