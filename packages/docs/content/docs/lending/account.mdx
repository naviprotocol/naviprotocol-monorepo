...
`getLendingState`, `getHealthFactor`, `getSimulatedHealthFactor`, and `getCoins` read on-chain
state, so they require an explicit v2 gRPC/Core client — the SDK's default client is a
placeholder that throws until you pass one. Construct a client once and reuse it:
...
const lendingState = await getLendingState('0x...', { client: grpcClient })
...
const healthFactor = await getHealthFactor('0x...', { client: grpcClient })
```
