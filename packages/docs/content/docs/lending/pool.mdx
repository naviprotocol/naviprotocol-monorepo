...
The "specific fee rate" mode of `getBorrowFee` reads on-chain state, so it requires an explicit
v2 gRPC/Core client — the SDK's default client is a placeholder that throws until you pass one.
...
Get information for all available lending pools. You can filter pools by market(s). See [documentation](/typedoc/variables/_naviprotocol_lending.getPools.html) for details.
...
const borrowFee = await getBorrowFee({
  ...
  env: 'prod', // Optional: environment configuration
  client: grpcClient, // Optional: Sui client instance for on-chain queries
  cacheTime: 30000, // Optional: cache expiration time in milliseconds
  market: 'main' // Optional: market identifier
})
```
