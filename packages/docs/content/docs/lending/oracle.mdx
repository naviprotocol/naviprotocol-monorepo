...
`getPythStalePriceFeedIdV2`, `updatePythPriceFeeds`, `updateOraclePricesPTB` (when
`updatePythPriceFeeds: true`), and `getLendingState` read on-chain state, so they require an
explicit v2 gRPC/Core client — the SDK's default client is a placeholder that throws until you
pass one. Construct a client once and reuse it:
...
const userLendingState = await getLendingState('0x...', { client: grpcClient })
...
// With a custom Sui client (e.g. a different network or a proxied endpoint)
await updateOraclePricesPTB(tx, allPriceFeeds, {
  updatePythPriceFeeds: true,
  client: grpcClient, // Optional: custom Sui client for on-chain price staleness detection
  env: 'prod' // Optional: environment configuration
})
```

<Callout>
  If you need to stay on the legacy JSON-RPC transport temporarily, construct the client via
  `legacyJsonRpc` (e.g. `createNaviSuiClientBundle({ network, grpc, legacyJsonRpc: { url } })`)
  instead of gRPC. JSON-RPC is deprecated and scheduled for removal on 2026-07-31; new
  integrations should use gRPC/Core clients like `SuiGrpcClient` directly.
</Callout>
...
const lendingState = await getLendingState(userAddress, { client: grpcClient })
