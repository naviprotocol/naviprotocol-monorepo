...
- Numeric ID (e.g., `0` for Main Market, `1` for Ember Market)
- String key (e.g., `'main'`, `'ember'`, `'sui-usdc'`)
- MarketConfig object
...
// Get by string key
const mainConfig = getMarketConfig('main')
const emberConfig = getMarketConfig('ember')
const suiUsdcConfig = getMarketConfig('sui-usdc')

// Get by numeric ID
const configById = getMarketConfig(0) // Main Market
const emberConfigById = getMarketConfig(1) // Ember Market
const suiUsdcConfigById = getMarketConfig(7) // SUI/USDC Market
...
// Get Ember Market
const emberMarket = await getMarket('ember', {
  env: 'prod'
})

// Get market by numeric ID
const marketById = await getMarket(0, { env: 'prod' }) // Main Market
const emberMarketById = await getMarket(1, { env: 'prod' }) // Ember Market
```
...
const markets = await getMarkets(['main', 'ember'], {
  env: 'prod', // Optional: environment configuration
  cacheTime: 30000 // Optional: cache time
})

// Use different identifiers
const marketsMixed = await getMarkets([0, 1, 'main', 'ember'], {
  env: 'prod'
})
```
...
// { id: 0, key: 'main', name: 'Main Market' }

// Access ember market
const emberMarket = MARKETS.ember
// { id: 1, key: 'ember', name: 'Ember Market' }

// Access isolated markets
const suiUsdcMarket = MARKETS['sui-usdc']
// { id: 7, key: 'sui-usdc', name: 'SUI/USDC Market' }
```

Available isolated market keys include `vsui-usdc`, `sui-usdc`, `hasui-sui`, `wbtc-usdc`, `vsui-sui`, and `xbtc-usdc`.
