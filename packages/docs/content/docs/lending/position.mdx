...
`getLendingPositions` (and the `getLendingState`/`getUserEModeCaps` calls used for comparison
below) read on-chain state, so they require an explicit v2 gRPC/Core client — the SDK's default
client is a placeholder that throws until you pass one. Construct a client once and reuse it:
...
const positions = await getLendingPositions('0x...', { client: grpcClient })
...
```typescript
const lendingState = await getLendingState('0x...', { client: grpcClient })
const emodeCaps = await getUserEModeCaps('0x...', { client: grpcClient })
