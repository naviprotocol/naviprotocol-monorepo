...
- **EMode support**: Includes EMode supply and borrow positions
...
Get user's lending positions including supply and borrow positions, with support for EMode positions. This function returns normalized amounts (using token decimals) and pre-calculated USD values. See [documentation](/typedoc/variables/_naviprotocol_lending.getLendingPositions.html) for details.
...
**getLendingState** doesn't include EMode positions directly - you need to query EMode caps separately:

```typescript
const lendingState = await getLendingState('0x...')
const emodeCaps = await getUserEModeCaps('0x...')
// Need to manually combine and process
```

**getLendingPositions** includes EMode positions automatically:
