...
`getUserAvailableLendingRewards` reads on-chain state, so it requires an explicit v2 gRPC/Core
client — the SDK's default client is a placeholder that throws until you pass one. Construct a
...
const rewards = await getUserAvailableLendingRewards('0x...', { client: grpcClient })
```
