...
- `client`: Sui SDK v2 client instance. Recommended: `SuiGrpcClient` from `@mysten/sui/grpc` (gRPC/Core). The legacy `SuiJsonRpcClient` from `@mysten/sui/jsonRpc` still works but JSON-RPC is deprecated (sunset 2026-07-31), so avoid it for new integrations. Note: on-chain read functions (e.g. `getLendingState`, `getUserAvailableLendingRewards`) require an explicit `client` — the default client throws if none is provided.
...
// The client is required for on-chain reads — construct it once and reuse it in the examples below
...
const lendingStates = await getLendingState(address, { client })
```
...
await getHealthFactor(address, { client })
```
<br/>
...
await getUserAvailableLendingRewards(address, { client })
```
...
const rewards = await getUserAvailableLendingRewards(address, { client });
