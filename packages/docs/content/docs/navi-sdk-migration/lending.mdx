...
- `client`: Sui SDK v2 client instance. Optional — it defaults to the public mainnet node (same as before); pass `{ client }` to use your own endpoint. Recommended: `SuiGrpcClient` from `@mysten/sui/grpc` (gRPC/Core). The legacy `SuiJsonRpcClient` from `@mysten/sui/jsonRpc` still works but JSON-RPC is deprecated (sunset 2026-07-31), so avoid it for new integrations.
