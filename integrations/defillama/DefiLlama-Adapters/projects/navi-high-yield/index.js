...
// NAVI Vault (`navi_vault`) — production deployment, "High Yield" vaults only.
//
...
// vaults into TVL.
//
// The two "Prime" (blue-chip) vaults of the same deployment are a separate
// DefiLlama listing (`navi-prime`), matching how NAVI's own Earn page splits
// them.
const VAULTS = [
  {
    id: "0x54359eb5d0e4364bd26989899fdb472f5594d1885e1f0d816ef4a066cab2ae4c",
    name: "USDC High Yield", // Vault<0xdba34672e30cb065b1f93e3ab55318768fd6fef66c15942c9f7cb846e2f900e7::usdc::USDC>, 6 decimals
  },
  {
    id: "0x864527a8ed2435aed828b46c6d9d0244506b418761cca25b7dd47a83c7797a29",
    name: "SUI High Yield", // Vault<0x2::sui::SUI>, 9 decimals
  },
]
...
const FALLBACK_COIN_TYPES = {
  "0x54359eb5d0e4364bd26989899fdb472f5594d1885e1f0d816ef4a066cab2ae4c":
    "0xdba34672e30cb065b1f93e3ab55318768fd6fef66c15942c9f7cb846e2f900e7::usdc::USDC",
  "0x864527a8ed2435aed828b46c6d9d0244506b418761cca25b7dd47a83c7797a29": "0x2::sui::SUI",
}
...
async function tvl(api) {
  ...
  vaults.forEach((vault, i) => {
    ...
    if (!vault) throw new Error(`NAVI High Yield: vault object not found: ${name} (${id})`)

    // `Vault<CoinType>` -> `CoinType`
    const coinType = vault.type?.replace(">", "").split("<")[1] || FALLBACK_COIN_TYPES[id]
    if (!coinType) throw new Error(`NAVI High Yield: could not resolve coin type for ${name} (${id})`)
    ...
    if (totalAssets === undefined) throw new Error(`NAVI High Yield: total_assets missing on ${name} (${id})`)
...
module.exports = {
  ...
  doublecounted: true,
  methodology:
    "Sums the `total_assets` field of NAVI's two High Yield vault objects on Sui: USDC High Yield and SUI High Yield. `total_assets` is each vault's idle balance plus the assets it has supplied into NAVI's lending markets, read directly from the vault object. Nothing is borrowed by the vaults, so nothing is netted out. Marked as double counted because the supplied assets are already included in the NAVI lending market TVL.",
  sui: {
    tvl,
  },
}
